...
export default {
    ...
    subSkill: {
      use: {
        ...
        async cost(event, trigger, player) {
          ...
          const result = await player
            .chooseBool(
              `酒诗：是否令${get.translation(current)}视为使用一张【酒】？`,
            )
            .set("ai", () => true)
            .forResult()
          event.result = {
        ...
        async content(event, trigger, player) {
          ...
          if (current?.isIn()) {
            await current.chooseUseTarget(
              new lib.element.VCard({ name: "jiu", isCard: true }),
              true,
            )
          }
        },
  ...
  gz_ol_zongpei: {
    ...
    getCards(event, player) {
      ...
      if (event.name === "lose") {
        ...
        const related =
          parent.name === "orderingDiscard" ? parent.relatedEvent : parent
        const source = related?.player || event.discarder
        if (
          !source ||
          source === player ||
          related?.name === "useCard" ||
          event.type === "use"
        ) {
          return []
        }
      ...
      return cards
        .filter(
          (card) =>
            get.position(card, true) === "d" &&
            lib.skill.gz_ol_zongpei.isZongpeiCard(card),
        )
        .toUniqued()
    },
  },
    ...
    hasTaoRecord(target) {
      if (
        ...
        game.getGlobalHistory(
          "useCard",
          (evt) => evt.card?.name === "tao" && evt.targets?.includes(target),
        ).length
      ) {
        return true
      }
  ...
  gz_ol_shantong: {
    ...
    async cost(event, trigger, player) {
      ...
      next.set(
        "prompt",
        "擅统：是否暗置一张武将牌，令此回合使用的下一张牌无距离和次数限制？",
      )
      next.set("ai", () => "副将")
      const result = await next.forResult()
      if (result.control !== "cancel2") {
