  ...
  binglinchengxiax: {
    ...
    async content(event, trigger, player) {
      ...
      await player
        .showCards(
          event.showCards,
          `${get.translation(player)}使用了【${get.translation(event.card)}】`,
          true,
        )
    ...
    ai: {
      ...
      result: {
        target(player, target, card, isLink) {
          ...
          if (evt && evt.card === card && evt.customArgs?.tiaojiyanmei_map) {
          ...
          if (!targets.length) {
            if (get.attitude(player, target) < 0) {
              ...
              if (
                game.hasPlayer(
                  (current) =>
                    current.countCards("h", fh) ===
                    target.countCards("h", fh) - 2,
                )
              ) {
                return -2
              }
              if (
                game.hasPlayer(
                  (current) =>
                    current.countCards("h", fh) < target.countCards("h", fh),
                )
              ) {
                return -1
              }
