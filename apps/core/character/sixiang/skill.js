...
const skills = {
  ...
  stdgaobian: {
    ...
    async content(event, trigger, player) {
      ...
      const num = get.attitude(target, player)
      const result = bool
        ? await target
            .chooseButton({
              createDialog: [
              ...
              ai(button) {
                ...
                return {
                  sha: Math.max(
                    ...cards.map((card) => player.getUseValue(card)),
                  ),
                  re: num,
                }[button.link]
              },
  ...
  stdyuqi: {
    ...
    filter(event, player) {
      return (
        get.distance(event.player, player) <= player.getHp() &&
        event.player.isIn()
      )
    },
    logTarget: "player",
  ...
  stdshuangrui: {
    ...
    async content(event, trigger, player) {
      await player
        ...
        .set(
          "openskilldialog",
          `###${get.prompt(event.name)}###将任意张手牌当作【杀】使用`,
        )
        .set("norestore", true)
            ...
            .step(async (event, trigger, player) => {
              ...
              if (
                trigger.targets?.some(
                  (target) => target.countCards("h") === num,
                )
              ) {
    ...
    async content(event, trigger, player) {
      ...
      if (cards.length <= 2) {
        ...
        result = await player
          ...
          .set("ai", (button) => {
            if (
              get.type(button.link) === "basic" &&
              !["shan", "du"].includes(get.name(button.link))
            ) {
              return 0
            }
      ...
      if (result?.links?.length) {
        ...
        await player.showCards(
          shown,
          `${get.translation(player)}展示了${get.translation(target)}的牌`,
        )
        const names = shown.map((card) => get.name(card, target))
        const viewAs = (info) =>
          get.autoViewAs({ name: info[2], nature: info[3], isCard: true })
        ...
        const result2 = await player
          .chooseButton(
            [`谮构：请选择要视为使用的基本牌`, [list, "vcard"]],
            true,
          )
          .set("viewAs", viewAs)
          .set("ai", (button) =>
            get.player().getUseValue(get.event().viewAs(button.link)),
          )
          .forResult()
  ...
  stdfeili: {
    ...
    filter(event, player) {
      return (
        ...
        (player.hasSkill("stdzengou") && !player.isTempBanned("stdzengou"))
      )
    },
    async cost(event, trigger, player) {
      ...
      if (bool1 && bool2) {
        ...
        const result = await player
          .chooseBool(get.prompt(event.skill), `${list[0]}，防止此次伤害`)
          .set("choice", ai1)
          .forResult()
        if (result?.bool) {
        ...
        const result = await player
          .chooseBool(get.prompt(event.skill), `${list[1]}，防止此次伤害`)
          .set("choice", ai2)
          .forResult()
        if (result?.bool) {
    ...
    filter(event, player) {
      ...
      if (event.player === player) {
        return event.targets.some(
          (target) => get.distance(target, player) === 1,
        )
      }
      return (
  ...
  stdkengqiang: {
    ...
    async cost(event, trigger, player) {
      const list = [
        `摸两张牌`,
        `失去1点体力，令${get.translation(trigger.card)}伤害+1`,
      ]
    ...
    filter(event, player) {
      return player
        ...
        .some((card) =>
          event.filterCard(
            get.autoViewAs(
              { name: get.name(card), nature: get.nature(card) },
              "unsure",
            ),
            player,
            event,
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        const list = get.inpileVCardList((info) => {
          return cards.some(
            (card) =>
              get.name(card) === info[2] && get.nature(card) === info[3],
          )
        })
      ...
      filter(button) {
        ...
        return evt.filterCard(
          get.autoViewAs({ name: link[2], nature: link[3] }, "unsure"),
          get.player(),
          evt,
        )
      },
          ...
          async precontent(event, trigger, player) {
            ...
            const result = await player
              .chooseCardButton(
                `评鉴：请移去一张“访客”`,
                true,
                player.getExpansions("stdyingmen"),
              )
  ...
  stdtongli: {
    ...
    filter(event, player) {
      ...
      const num = game
        .filterPlayer()
        .flatMap((i) => i.getCards("ej").map((card) => get.suit(card)))
        .unique().length
      return (
        player.isPhaseUsing() &&
        player.getHistory(
          "useCard",
          (evt) => evt.getParent("phaseUse") === evtx,
        ).length === num &&
    ...
    check(event, player) {
      return (
        ...
        (event.targets?.reduce(
          (sum, i) => sum + get.effect(i, event.card, player, player),
          0,
        ) <
          0)
      )
      ...
      draw: {
        ...
        filter(event, player) {
          ...
          const check = (card) =>
            ...
            get.number(card) === get.number(event.card)
          return (
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set(
          "ai",
          (target) =>
            get.attitude(get.player(), target) * target.countCards("he"),
        )
        .forResult()
    },
    async content(event, trigger, player) {
          ...
          .chooseButtonTarget({
            ...
            ai1(button) {
              ...
              if (
                !game.hasPlayer(
                  (current) =>
                    player !== current && get.attitude(player, current) > 0,
                )
              ) {
    ...
    onChooseToUse(event) {
      ...
      if (!game.online && !event.stdkuangcai && event.type === "phase") {
        event.set(
          ...
          player.getHistory(
            "useCard",
            (evt) => evt.getParent("phaseUse") === event.getParent("phaseUse"),
          ).length,
        )
      }
    },
    ...
    async content(event, trigger, player) {
      ...
      while (true) {
        ...
        await player
          .showCards(
            card,
            `${get.translation(player)}【工巧】亮出的第${get.cnNumber(cards.length, true)}张牌`,
            true,
          )
  ...
  stdjinhui: {
    ...
    async content(event, trigger, player) {
      ...
      await player
        .showCards(cards, `${get.translation(player)}发动了【锦绘】`, true)
        .set("clearArena", false)
      const targets = [player]
      if (game.hasPlayer((target) => target !== player)) {
        const result = await player
          .chooseTarget(
            `锦绘：与一名其他角色依次使用其中一张牌（不能连续使用相同颜色的牌）`,
            true,
            lib.filter.notMe,
          )
      ...
      for (const target of targets) {
        if (
          cards.some(
            (card) =>
              target.hasUseTarget(card, true, false) &&
  ...
  stdfuli: {
    ...
    async content(event, trigger, player) {
      const func = async (target) => {
        ...
        const hs = target.getDiscardableCards(
          target,
          "h",
          (card) => get.tag(card, "damage") && get.type(card) !== "delay",
        )
        if (hs.length) {
      ...
      if (game.hasPlayer((target) => target !== player)) {
        const result = await player
          ...
          .set("ai", (target) =>
            get.recoverEffect(target, get.player(), get.player()),
          )
          .forResult()
        if (result?.targets?.length) {
          const [target] = result.targets
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          if (
            game.hasPlayer(
              (target) => get.recoverEffect(target, player, player) > 0,
            )
          ) {
            return 1
          }
  ...
  stddehua: {
    ...
    filter(event, player) {
      return (
        player
          .getHistory("lose")
          .reduce((list, evt) => list.addArray(evt.cards), []).length === 2
      )
    },
    async cost(event, trigger, player) {
      const list = get.inpileVCardList((info) => {
      ...
      return game.hasPlayer2(
        (target) =>
          ...
          event
            ...
            ?.cards2.some(
              (card) =>
                get.suit(card) === "heart" && get.position(card, true) === "d",
            ),
      )
    },
    async cost(event, trigger, player) {
  ...
  stdyuanzhuo: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) =>
        lib.skill.stdyuanzhuo.filterTarget(void 0, player, target),
      )
    },
    filterTarget(card, player, target) {
  ...
  stdbizun: {
    ...
    hiddenCard(player, name) {
      return (
        ...
        player.countCards("hes", (card) => get.type(card, player) === "equip")
      )
    },
    filter(event, player) {
      return (
            ...
            player
              ...
              .step(async (event, trigger, player) => {
                const target = game.findPlayer(
                  (targetx) =>
                    targetx.canMoveCard() && targetx.isMaxHandcard(true),
                )
                if (target) {
        ...
        .set("ai", (target) => {
          ...
          if (att > 0) {
            return target.countCards("h", (card) =>
              target.hasValueTarget(card, true, false),
            )
          }
          return true
        })
    ...
    async content(event, trigger, player) {
      ...
      for (const target of targets.sortBySeat()) {
        const result = await player
          .choosePlayerCard(
            target,
            `励军：请展示${get.translation(target)}一张手牌`,
            "h",
            true,
          )
          .forResult()
        if (result?.cards?.length) {
          ...
          if (target.hasUseTarget(card, true, false)) {
            resultx = await target
              .chooseUseTarget(
                `励军：使用${get.translation(card)}或者取消并弃置之`,
                card,
                false,
              )
              .forResult()
    ...
    filter(event, player) {
      return player.countCards(
        ...
        (card) =>
          ...
          game.hasPlayer((target) => target.canEquip(card, true)),
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
  ...
  stdleiruo: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (target) => target !== player && target.countGainableCards(player, "e"),
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    async content(event, trigger, player) {
      ...
      if (target.canUse(card, player, false, false)) {
        const result = await target
          .chooseBool(
            `羸弱：是否视为对${get.translation(player)}使用一张无距离限制的【杀】`,
          )
      ...
      const result = await player
        ...
        .set("ai", (card) => {
          const player = get.player(),
            ...
            att = get.attitude(player, target)
          if (att <= 0) {
            const num = target.countCards(
              "h",
              (card) => !target.hasValueTarget(card),
            )
            if (num >= target.hp) {
              return 6 - get.value(card)
            }
  ...
  xuanhuafu_skill: {
    ...
    async cost(event, trigger, player) {
      ...
      if (targets.length === 1) {
        ...
        event.result = await player
          .chooseTarget(
            `宣花斧：为${get.translation(card)}额外指定一个目标`,
            true,
            (card, player, target) => {
  ...
  baipishuangbi_skill: {
    ...
    filter(event, player) {
      return (
        event.card.name === "sha" &&
        player.differentSexFrom(event.target) &&
        !player.countCards("h")
      )
    },
    async content(event, trigger, player) {
  ...
  stdruixi: {
    ...
    async content(event, trigger, player) {
      await player
        ...
        .set(
          "openskilldialog",
          `###${get.prompt(event.name)}###将一张牌当作无距离限制的【杀】使用`,
        )
        .set("norestore", true)
        .set("_backupevent", `${event.name}_backup`)
        .set("custom", {
  ...
  stdzhitu: {
    ...
    filter(event, player) {
      ...
      return get.inpileVCardList((info) => {
        ...
        return event.filterCard(
          get.autoViewAs({ name: info[2] }, "unsure"),
          player,
          event,
        )
      }).length
    ...
    chooseButton: {
      ...
      filter(button, player) {
        return get
          .event()
          .getParent()
          .filterCard({ name: button.link[2] }, player, get.event().getParent())
      },
      check(button) {
        const player = get.player()
      ...
      backup(links, player) {
        return {
          ...
          filterCard(card, player) {
            ...
            return (
              get.number(card, player) +
                selected.reduce(
                  (sum, card) => sum + get.number(card, get.player()),
                  0,
                ) <=
              13
            )
          ...
          ai1(card) {
            ...
            const sum = ui.selected.cards.reduce(
              (sumx, cardx) => sumx + get.number(cardx, player),
              0,
            )
  ...
  stdyizheng: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (target) => target.hp >= player.hp && player.canCompare(target),
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          const player = get.player(),
            hs = player.getCards("h").sort((a, b) => b.number - a.number),
            ts = target.getCards("h").sort((a, b) => b.number - a.number),
  ...
  stdtaoluan: {
    ...
    async content(event, trigger, player) {
      ...
      await player.give(cards, target)
      await target.showHandcards()
      await target.modedDiscard(
        target.getCards("h", (card) => get.name(card, target) === "shan"),
      )
    },
  },
  //标笮融
  stdcansi: {
    ...
    filter(event, player) {
      return (
        player.countCards("he") &&
        game.hasPlayer((target) => player.inRange(target))
      ) //&&player.countGainableCards(target,"he")
    },
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget(
          `残肆：令攻击范围内的一名角色获得你一张牌，然后视为对其依次使用【杀】,【决斗】`,
          true,
          (card, player, target) => {
        ...
        .set("ai", (target) => {
          ...
          return cards.reduce(
            (eff, card) =>
              eff + get.effect(target, card, get.player(), get.player()),
            0,
          )
        })
        .forResult()
  ...
  stdlingjian: {
    ...
    filter(event, player) {
      return (
        ...
        !player.hasHistory(
          "sourceDamage",
          (evt) => evt.card && evt.getParent("useCard") === event,
        ) &&
  ...
  stdxianlu: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) =>
        target.countCards("e", { type: "equip" }),
      )
    },
    filterTarget(card, player, target) {
  ...
  stdgangjian: {
    ...
    check(event, player) {
      ...
      if (
        player.hasSkill("stdguijie") &&
        player.countCards("hes", { color: "red" }) > 1
      ) {
        return true
      }
      if (
    ...
    async content(event, trigger, player) {
      ...
      if (target.canUse({ name: "sha" }, player)) {
        ...
        if (
          !game.hasPlayer2(
            (current) =>
              current.hasHistory("damage", (evt) => evt.getParent(3) === event),
            true,
          )
        ) {
    ...
    ai: {
      order(item, player) {
        if (
          player.countCards(
            "hes",
            (card) => get.color(card) === "red" && get.name(card) !== "shan",
          ) > 3
        ) {
          return 7
        }
  ...
  stdtianming: {
    ...
    check(event, player) {
      ...
      if (
        hs.length <= 2 &&
        hs.some((card) => ["shan", "tao"].includes(card.name))
      ) {
        return false
      }
  ...
  stdmizhao: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") &&
        game.countPlayer((current) => player !== current) > 1
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    async content(event, trigger, player) {
      ...
      const result = await target1
        .chooseBool(
          ...
          `与${get.translation(target2)}各失去1点体力？`,
        )
        .set(
          "choice",
  ...
  stdwufei: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (current) => current.hasSex("female") && current.countCards("h"),
      )
    },
    async cost(event, trigger, player) {
      ...
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          const bool = Object.keys(lib.color).some((color) => {
            const num = target.countCards(
              "h",
              (card) => get.color(card, target) === color,
            )
            return num > 0 && num <= 2
          })
          return att * (bool ? 1 : 0)
    ...
    async content(event, trigger, player) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (cards.length) {
          dialog.addArray([
            `<span class="text center">${get.translation(colorx)}</span>`,
            cards,
          ])
        }
      }
      ...
      if (control) {
        ...
        const cards = target
          .getCards("h")
          .filter((card) => get.color(card) === control)
        if (cards.length) {
  ...
  stdduwu: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") &&
        game.hasPlayer((current) =>
          get.info("stdduwu").filterTarget(null, player, current),
        )
      )
    },
    filterCard: true,
        ...
        if (
          game.hasPlayer(
            (current) =>
              ...
              get.effect(current, "stdduwu", player, player) > 0,
          ) &&
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          if (
            !target.countCards("e") &&
            target.countCards("h") - 1 === target.getHp()
          ) {
            return get.effect(target, { name: "guohe_copy2" }, player, player)
          }
          return (
  ...
  stdzhennan: {
    ...
    filter(event, player) {
      return (
        event.player !== player && player.countDiscardableCards(player, "h")
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    getCards(event) {
      return game
        .getGlobalHistory("everything", (evt) => {
          ...
          return (
            (evt.name === "cardsDiscard" || evt.position === ui.discardPile) &&
    ...
    filter(event, player) {
      ...
      return (
        player.countCards("hes", (card) =>
          get.info("stdqingcheng").filterCard(card, player),
        ) > 1
      )
    },
    filterCard(card, player) {
      return (
        get.type2(card, player) !== "trick" && get.color(card, player) === "red"
      )
    },
    position: "he",
    selectCard: 2,
    ...
    async content(event, trigger, player) {
      ...
      for (let i = 0; i < cards.length; i++) {
        const card = get.autoViewAs({ name: "lebu", cards: [cards[i]] }),
          target = targets[i]
        if (
          (player === target && player.canAddJudge(card)) ||
          (player !== target && player.canUse(card, target))
        ) {
  ...
  stdmeibu: {
    ...
    filter(event, player) {
      return (
        event.player.countDiscardableCards(event.player, "h") &&
        event.player.getEquips(1).length &&
        event.card?.name === "sha"
      )
    },
    logTarget: "player",
  ...
  stdqijian: {
    ...
    filter(event, player) {
      return game.hasPlayer((current) =>
        game.hasPlayer(
          (currentx) =>
            current.countCards("h") + currentx.countCards("h") === 7,
        ),
      )
    },
    async cost(event, trigger, player) {
      ...
      for (const target of targets) {
        ...
        if (goon) {
          result = await target
            ...
            .set(
              "prompt",
              `七笺：弃置${get.translation(targetx)}一张牌或令其摸一张牌`,
            )
            .set("ai", () => {
        ...
        if (control === "摸牌") {
          ...
          control === "弃牌" &&
          targetx.countDiscardableCards(target, "he")
        ) {
  ...
  stdyoudi: {
    ...
    subSkill: {
      backup: {
        filterCard(card, player) {
          return (
            get.itemtype(card) === "card" && get.color(card, player) === "red"
          )
        },
        position: "hes",
  ...
  stdjuezhu: {
    ...
    filter(event, player, name) {
      ...
      return (
        event.source?.isIn() &&
        player.canUse({ name: "juedou", isCard: true }, event.source)
      )
    },
    forced: true,
    async content(event, trigger, player) {
  ...
  stdchengji: {
    ...
    filterCard(card, player) {
      ...
      return ui.selected.cards.every(
        (cardx) => get.color(cardx, player) !== get.color(card, player),
      )
    },
    complexCard: true,
    position: "hes",
    ...
    viewAsFilter(player) {
      ...
      return (
        _status.connectMode ||
        player.getCards("hes").some((card) => get.color(card, player) !== color)
      )
    },
    ...
    ai: {
      skillTagFilter(player) {
        ...
        return (
          ...
          player
            .getCards("hes")
            .some((card) => get.color(card, player) !== color)
        )
      },
      respondSha: true,
    },
        ...
        async content(event, trigger, player) {
          ...
          while (true) {
            ...
            if (event.current.group === "shu") {
              const discardEvent = event.current.chooseToDiscard(
                `是否弃置一张牌，视为${get.translation(player)}打出一张杀？`,
              )
              discardEvent.set(
                "filterCard",
              ...
              discardEvent.set(
                "skillwarn",
                `弃置一张牌，视为${get.translation(player)}打出一张杀`,
              )
  ...
  stdchoulve: {
    ...
    async content(event, trigger, player) {
      ...
      const { bool, cards: cardx } = await target
        .chooseCard(
          `是否展示并交给${get.translation(player)}一张装备牌？`,
          "he",
        )
  ...
  stdjiaozhao: {
    ...
    async content(event, trigger, player) {
      ...
      if (event.getParent(2).name === "stddanxin") {
        ...
        const cardx = player
          .getCards("h")
          .sort((a, b) => player.getUseValue(a) - player.getUseValue(b))[0]
        const result = await player
          .chooseButton(2, [
            "你的手牌",
            player.getCards("h"),
        ...
        player(player, target) {
          ...
          if (player.hasSha() && geteff(target)) {
            var num = game.countPlayer(
              (current) =>
                current !== player && player.inRange(target) && geteff(current),
            )
            if (num === 0) {
              if (
                game.hasPlayer(
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set(
          ...
          (() => {
            let eff = 0
            for (const cur of trigger.targets) {
  ...
  stdchuyi: {
    ...
    round: 1,
    filter(event, player) {
      if (!event.source?.isIn() || event.source === player) {
        return false
      }
    ...
    check(event, player) {
      return (
        ...
        get.damageEffect(event.player, event.source, player, event.nature) > 0
      )
    },
    async content(event, trigger, player) {
      trigger.num++
  ...
  stdshuangdao: {
    ...
    async cost(event, trigger, player) {
      ...
      if (player.needsToDiscard() > 1) {
        goon = player.hasCard(
          (card) => card.number > 10 && get.value(card) <= 5,
        )
      } else {
        goon = player.hasCard(
          (card) =>
            (card.number >= 9 && get.value(card) <= 5) || get.value(card) <= 3,
        )
      }
      event.result = await player
        .chooseTarget(get.prompt2(event.skill), (card, player, target) =>
      ...
      if (result.bool) {
        if (
          ...
          const result2 = await player
            ...
            .forResult()
          if (result2.bool) {
            for (const targetx of result2.targets) {
  ...
  stdzongji: {
    ...
    filter(event, player) {
      ...
      if (!event.player.isIn() || !event.source?.isIn()) {
        return false
      }
  ...
  stdyuanqing: {
    ...
    filter(event, player) {
      const targets = lib.skill.stdyuanqing.logTarget(event, player)
      return targets?.length
    },
    logTarget(event, player) {
      return game.filterPlayer((current) => {
        const cards = lib.skill.stdyuanqing.getCards(current)
        return cards?.length
      })
    },
    async content(event, trigger, player) {
      for (const target of event.targets) {
        const cards = lib.skill.stdyuanqing.getCards(target)
        if (!cards.length) {
          continue
        }
        const result = await target
          .chooseButton(["获得其中一张牌", cards], true)
          .forResult()
        if (result.bool) {
  ...
  stdgushe: {
    ...
    async content(event, trigger, player) {
      ...
      while (num > 0 && player.canCompare(target)) {
        ...
        if (player.canCompare(target)) {
          for (const loser of failure) {
            const choice =
              loser.countCards(
                "h",
                (card) => get.value(card) <= 6 && card.number > 10,
              ) > 0
            const { bool } = await loser
              .chooseBool("是否与其再次拼点？")
              .set("choice", choice)
              .forResult()
            if (bool) {
  ...
  stdjici: {
    ...
    check(event, player) {
      return (
        (player.getHp() < 2 &&
          player.getHp() + player.countCards("h", { name: ["tao", "jiu"] }) >
            2) ||
        (player.getHp() > 1 &&
          player.getHp() + player.countCards("h", { name: "tao" }) > 2)
      )
  ...
  stdyinge: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        ...
        .set("filterTarget", (card, player, target) => {
          const targets = get.event().useTargets
  ...
  stdjuyi: {
    ...
    filter(event, player) {
      ...
      if (
        player.hasHistory(
          "damage",
          (evt) => evt.source && evt.source === event.source,
        )
      ) {
        return false
      }
      return (
        ...
        !player.getStorage("stdjuyi").includes(event.source)
      )
    },
    async cost(event, trigger, player) {
  ...
  stdwuyan: {
    ...
    forced: true,
    filter(event, player) {
      if (event.cards?.length !== 1 || get.type2(event.cards[0]) !== "trick") {
        return false
      }
  ...
  stdjujian: {
    ...
    usable: 1,
    filter(event, player) {
      return event.cards?.length && event.card.name === "wuxie"
    },
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          const player = get.player()
          return (
            target.getUseValue(_status.event.getTrigger().cards[0]) *
  ...
  oldlonghun: {
    ...
    inherit: "xinlonghun",
    prompt: () =>
      `将${get.cnNumber(Math.max(1, get.player().getHp()))}张♦牌当做杀，♥牌当做桃，♣牌当做闪，♠牌当做无懈可击使用或打出`,
    ...
    subSkill: {
      global: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer(
            (current) => current.hasSkill("stdxiemu") && current !== player,
          )
        },
        filterTarget(card, player, target) {
        ...
        selectTarget() {
          const num = game.countPlayer(
            (current) =>
              current.hasSkill("stdxiemu") && current !== get.player(),
          )
          return num > 1 ? 1 : -1
        },
        filterCard(card) {
  ...
  stdruwu: {
    ...
    enable: "chooseToUse",
    filter(event, player) {
      if (!event.stdruwu?.length) {
        return false
      }
      if (
        event.filterCard(
          get.autoViewAs({ name: "juedou" }, "unsure"),
          player,
          event,
        )
      ) {
      ...
      if (
        event.filterCard(
          get.autoViewAs({ name: "wuzhong" }, "unsure"),
          player,
          event,
        )
      ) {
    ...
    onChooseToUse(event) {
      ...
      var history = game.getGlobalHistory(
        "everything",
        (evt) => evt.player === event.player && evt.name === "equip",
      )
      list = list.filter((card) => {
        return !history.some((evt) => evt.cards?.includes(card))
      })
        ...
        .step(async (event, trigger, player) => {
          ...
          if (cards.length) {
            const { bool, links } = await player
              .chooseButton(["景略：是否获得本阶段弃置的一张牌？", cards])
              .forResult()
            if (bool) {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set(
          ...
          (() =>
            colors.sort((a, b) => {
              return (
                player.countCards("h", { color: a }) -
                player.countCards("h", { color: b })
              )
            })[0])(),
        )
        .forResult()
      ...
      if (discards.length) {
        ...
        if (
          ...
          const result2 = await player
            .chooseTarget(
              `获得至多${discards.length}名其他角色各一张牌`,
              [1, discards.length],
              true,
              (card, player, target) =>
  ...
  stdqiuyuan: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        .chooseToGive(
          `交给${get.translation(player)}一张牌，或成为${get.translation(card)}的额外目标`,
          player,
        )
        .set("ai", (card) => {
          const { player, target } = get.event()
  ...
  stdzhuikong: {
    ...
    filter(event, player) {
      ...
      return (
        (_status.connectMode && player.hasCards("h")) ||
        player.hasCards("h", (card) => get.name(card) === "sha")
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
  ...
  stdyouji: {
    ...
    filter(event, player) {
      return player.canMoveCard(
        ...
        (card) => {
          return [3, 4, 6].includes(
            parseInt(get.subtype(card)?.slice("equip".length), 10),
          )
        },
        "nojudge",
      )
    ...
    async content(event, trigger, player) {
      await player
        .moveCard({
          ...
          filter(card) {
            return [3, 4, 6].includes(
              parseInt(get.subtype(card)?.slice("equip".length) ?? "0", 10),
            )
          },
        })
        .set("nojudge", true)
  ...
  stdfengpo: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set(
          "prompt",
          `凤魄：弃置${target !== player ? get.translation(target) : ""}一张牌`,
        )
        .set("prompt2", "若弃置了方片牌，则此伤害+1")
        .set("att", get.sgnAttitude(player, target))
        .forResult()
      if (result.bool) {
        if (result.cards?.some((i) => get.suit(i, target) === "diamond")) {
          player.popup("洗具")
          trigger.num++
        }
      }
      ...
      effect: {
        ...
        mod: {
          cardEnabled(card, player) {
            if (
              player.getStorage("stddaoshu_effect").includes(get.suit(card))
            ) {
              return false
            }
          },
          ...
          if (targets.length) {
            for (const target of targets) {
              ...
              const next = target.chooseToUse(
                `挥战：是否替${get.translation(trigger.player)}使用一张【闪】？`,
                function (card) {
  ...
  stdjinjian: {
    ...
    async content(event, trigger, player) {
      ...
      player.addMark(
        `stdjinjian_effect${event.triggername.slice(11)}`,
        1,
        false,
      )
    },
      ...
      effect: {
        target(card, player, target) {
          ...
          if (get.tag(card, "damage")) {
            ...
            if (!target.getStorage("stdjinjian_used").includes("4")) {
              if (get.attitude(player, target) > 0) {
                return [0, 0.2]
              }
              if (get.attitude(player, target) < 0) {
  ...
  stdchunlao: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          get.prompt(event.skill),
          `用${get.translation(cards)}交换一名其他角色的手牌`,
          (card, player, target) => target !== player,
        )
        .set("ai", (target) => {
  ...
  stdshefu: {
    ...
    subSkill: {
      effect: {
        ...
        filter(event, player) {
          return player
            .getExpansions("stdshefu")
            .some((card) => card.name === event.card.name)
        },
        async cost(event, trigger, player) {
          const result = await player
            .chooseButton([
              `###${get.prompt("stdshefu")}###弃置一张同名牌，令此牌无效`,
              player.getExpansions("stdshefu"),
            ])
            .set("filterButton", (button) => {
            ...
            .set(
              "goon",
              lib.skill.sbkanpo.subSkill.kanpo.check(trigger, player),
            )
            .forResult()
  ...
  stdyibing: {
    ...
    check(event, player) {
      return (
        get.effect(event.player, { name: "shunshou_copy2" }, player, player) > 0
      )
    },
    async content(event, trigger, player) {
      await player.gainPlayerCard(
        trigger.player,
        "he",
        `获得${get.translation(trigger.player)}一张牌`,
        true,
      )
    },
  },
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          if (
            target.countMark("stdzhanying_count") ===
  ...
  stdzhanying: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        event.player.countMark("stdzhanying_count")
      )
    },
    forced: true,
    logTarget: "player",
    ...
    subSkill: {
      ...
      mark: {
        ...
        mod: {
          cardEnabled(card, player) {
            if (!_status.currentPhase?.hasSkill("stdzhanying")) {
              return
            }
            if (
              get.color(card) === "red" &&
              player.countMark("stdzhanying_count") < player.countCards("h")
            ) {
              return false
            }
          },
  ...
  stdtiaohe: {
    ...
    filter(event, player) {
      return game.hasPlayer((tar1) => {
        return (
          tar1.countDiscardableCards(
            player,
            "e",
            (i) => get.subtype(i) === "equip2",
          ) &&
          game.hasPlayer((tar2) => {
    ...
    filterTarget(card, player, target) {
      if (
        ...
        ui.selected.targets[0].countDiscardableCards(
          player,
          "e",
          (i) => get.subtype(i) === "equip2",
        )
    ...
    async content(event, trigger, player) {
      ...
      if (targets.length === 1) {
        await player
          ...
          .set("filterButton", (button) => {
            ...
            if (ui.selected.buttons.length) {
              const pos = get.position(ui.selected.buttons[0].link),
      ...
      for (let i = 0; i < 2; i++) {
        if (
          i &&
          canfj.includes(targets[i]) &&
          !targets[i].countDiscardableCards(
            player,
            "e",
            (i) => get.subtype(i) === "equip2",
          )
